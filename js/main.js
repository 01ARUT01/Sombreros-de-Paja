/* ============================================================
   LA LEYENDA DE ONE PIECE — JavaScript
   1. Sombreros de paja dorados en el hero (canvas)
...
/* ---------- 1. Sombreros de paja flotando (el guiño a Luffy) ---------- */
(function butterflies() {
  ...
  function makeButterfly() {
    return {
      ...
      size: 5 + Math.random() * 8,          // tamaño del sombrero
      angle: Math.random() * Math.PI * 2,   // dirección de deriva
      speed: 0.25 + Math.random() * 0.55,
      flap: Math.random() * Math.PI * 2,    // fase del balanceo
  ...
  function draw(b) {
    // Sombrero de paja: ala elíptica + copa con cinta roja
    ctx.save();
    ctx.translate(b.x, b.y);
    ctx.rotate(Math.sin(b.flap * 0.5) * 0.35); // balanceo suave
    ctx.globalAlpha = b.alpha;
    ctx.fillStyle = "#f2c14e";

    // Ala del sombrero
    ctx.beginPath();
    ctx.ellipse(0, b.size * 0.25, b.size * 1.35, b.size * 0.42, 0, 0, Math.PI * 2);
    ctx.fill();

    // Copa
    ctx.beginPath();
    ctx.ellipse(0, -b.size * 0.15, b.size * 0.8, b.size * 0.7, 0, Math.PI, Math.PI * 2);
    ctx.closePath();
    ctx.fill();

    // Cinta roja
    ctx.globalAlpha = b.alpha * 0.95;
    ctx.fillStyle = "#c8502e";
    ctx.fillRect(-b.size * 0.78, -b.size * 0.42, b.size * 1.56, b.size * 0.3);
